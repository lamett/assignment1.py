...
array_cities = read_coordinate_file("HungaryCities.txt")

# array mit verbundenen städten, array mit deren abständen erstellen
indices, distances = construct_graph_connections(array_cities, 0.005)

#csr-matrix erstellen
graph = construct_graph(indices, distances, max(np.shape(array_cities)))

#shortest path
path, dist = find_shortest_path(graph, 311, 702)
